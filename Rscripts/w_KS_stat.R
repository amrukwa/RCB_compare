w_KS_stat <- function(RCB_ctrl, RCB_exp, scale=0.5, ifplot=F, grp_ctrl_name="Control", grp_exp_name="Experimental") {
...
# Author: Michal Marczyk (michal.marczyk@yale.edu; michal.marczyk@polsl.edu)
  ...
  if (ifplot){
    ...
    p <- ggplot(data_plot, aes(x=x, y=ecdf_diff)) + geom_step(col="green") + theme_bw() +
